...
def get_metrics() -> "AgentMetrics":
    """获取全局 AgentMetrics 实例（被 trace_agent_call 装饰器回填）"""
    global _metrics
    if _metrics is None:
...
def trace_agent_call(agent_name: str) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            # OTel 未安装/未初始化时，仍然收集 AgentMetrics 指标
            if tracer is None:
                start_time = time.time()
                try:
                    result = await func(*args, **kwargs)
                    metrics.record_call(agent_name, (time.time() - start_time) * 1000, True)
                    return result
                except Exception:
                    metrics.record_call(agent_name, (time.time() - start_time) * 1000, False)
                    raise
            ...
            with tracer.start_as_current_span(span_name) as span:
                ...
                try:
                    ...
                    metrics.record_call(agent_name, duration_ms, True)
                    ...
                    metrics.record_call(agent_name, duration_ms, False)
                    raise
