...
from agents.knowledge_rag_hybrid import KnowledgeRAGAgent
...
class SupervisorNode:
    ...
    async def route_decision(self, state: AgentState) -> AgentState:
        ...
        if intent not in valid_intents:
            intent = "knowledge_rag"                 # 不在有效意图，默认是知识库里面的
    ...
    async def synthesize_response(self, state: AgentState) -> AgentState:
        ...
        if not compliance_passed:
            ...
            for agent_name, result in sub_results.items():
                if not result:
                    continue
                # 只把字符串型的子Agent输出拼到最终回复里；
                # 像 intent_router 这样写入元数据(dict)的项跳过。
...
def create_supervisor_graph(
    ...
    if llm is None:
        llm = ChatOpenAI(
            ...
            streaming=True,
            stream_usage=True,      # 流式下也返回 token 用量
        )
    ...
    wiki_pages_dir = str(wiki.wiki_dir) if wiki else None
    knowledge_agent = KnowledgeRAGAgent(llm, wiki_pages_dir=wiki_pages_dir)
